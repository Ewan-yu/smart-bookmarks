...
| 网页信息采集 | 90% | ⚠️ 基本完成 | content script 静默采集，但未保存到DB |
...
- [ ] **SYNC_BOOKMARKS 实现**：background.js 中该消息处理标注为 TODO，需要实现"同步到浏览器收藏夹"逻辑
- [ ] **content script 数据落盘**：collector.js 采集了页面信息并发送 `PAGE_INFO_COLLECTED` 消息，但 background.js 未处理该消息，采集数据未写入 DB
- [ ] **插件图标**：icons/ 目录需要补充实际的 16/48/128 尺寸图标
...
| 2 | `PAGE_INFO_COLLECTED` 消息未在 background 处理 | P0 | 待修复 |
| 3 | `checkSingleLink()` 函数体为 TODO | P2 | 待实现 |
| 4 | Git remote 凭据认证需手动指定用户名 | - | 已 workaround |
...
```
Week 1: P0（浏览器加载测试 → 发现并修复 JS 错误 → 补图标）
Week 2: P0（SYNC_BOOKMARKS implementation + content script 数据落盘）
Week 3: P1（uncertain 优化 + 错误提示 + 性能验证）
Week 4: P2（单条检测 + 分类/标签 CRUD + 导出入口）
